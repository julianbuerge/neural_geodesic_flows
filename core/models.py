...
class Classification(eqx.Module):
    #neural network that does classification using a tangentbundle and a MLP

    #the architecture uses psi, exp_g and a classification head nn
    #   input --> psi --> exp_g --> nn --> output

    layers: list    #for the classification head
    ...
    def __init__(self, tangentbundle, arguments, key = jax.random.PRNGKey(0)):

        #verify that essentialkeys are provided
        required_keys = ['amount_classes','hidden_sizes']
        for dict_key in required_keys:
            if dict_key not in arguments:
                raise ValueError(f"Missing required argument: '{dict_key}'")

        #save the tangentbundle
        self.tangentbundle = tangentbundle

        #initialize random keys for the classification head
        keys = jax.random.split(key, len(arguments['hidden_sizes']) + 1)

        #construct layer sizes based on geodesic solver output, hidden layers, and output size = amount of classes
        layer_sizes = [2*tangentbundle.dim_M] + arguments['hidden_sizes'] + [arguments['amount_classes']]
        ...
        self.classname = "Classification"


    #give all the high level parameters as a dictionary. this is required when we want to load a saved instance later
    def get_high_level_parameters(self):

        params_tangentbundle = self.tangentbundle.get_high_level_parameters()

        params_extended = {'classification_head_arguments' : self.arguments}

        #join both dictionaries (if a key appears in both only those from extended are kept)
        params = params_tangentbundle.update(params_extended)
    ...
    def __call__(self, y, t, num_steps : int):

        #first we go from the data space to the manifold
        xv = self.tangentbundle.psi(y)

        #second we evolve along a geodesic until time t
        xv_evolved = self.tangentbundle.exp(xv, t, num_steps)

        #third we go through the classification head
        c = self.nn_head(xv_evolved)

        #fourth we take a log softmax activation to get log probabilites of each class
        p = jax.nn.log_softmax(c)
